...
func (this *PreferenceController) RegisterRoutes() map[string]func(writer http.ResponseWriter, request *http.Request) {
	...
	routeMap["/api/preference/ping"] = this.Wrap(this.Ping, model.USER_ROLE_GUEST)
	routeMap["/api/preference/fetch"] = this.Wrap(this.Fetch, model.USER_ROLE_GUEST)
	routeMap["/api/preference/edit"] = this.Wrap(this.Edit, model.USER_ROLE_ADMINISTRATOR)
	routeMap["/api/preference/edit/preview/config"] = this.Wrap(this.EditPreviewConfig, model.USER_ROLE_ADMINISTRATOR)
	routeMap["/api/preference/edit/scan/config"] = this.Wrap(this.EditScanConfig, model.USER_ROLE_ADMINISTRATOR)
	routeMap["/api/preference/scan/once"] = this.Wrap(this.ScanOnce, model.USER_ROLE_ADMINISTRATOR)
	routeMap["/api/preference/system/cleanup"] = this.Wrap(this.SystemCleanup, model.USER_ROLE_ADMINISTRATOR)
...
func (this *PreferenceController) Edit(writer http.ResponseWriter, request *http.Request) *result.WebResult {
	...
	if allowRegisterStr == model.TRUE {
		allowRegister = true
	}
...
func (this *PreferenceController) EditScanConfig(writer http.ResponseWriter, request *http.Request) *result.WebResult {
	...
	if scanConfig.Enable {
		...
		//validate scope.
		if scanConfig.Scope == model.SCAN_SCOPE_CUSTOM {
			if len(scanConfig.SpaceNames) == 0 {
				panic(result.BadRequest("scope cannot be null"))
			}
		} else if scanConfig.Scope == model.SCAN_SCOPE_ALL {
