...
func (this *ShareController) Init() {
	...
	if b, ok := b.(*service.AlienService); ok {
		this.alienService = b
	}
...
func (this *ShareController) RegisterRoutes() map[string]func(writer http.ResponseWriter, request *http.Request) {
	...
	routeMap["/api/share/create"] = this.Wrap(this.Create, model.USER_ROLE_USER)
	routeMap["/api/share/delete"] = this.Wrap(this.Delete, model.USER_ROLE_USER)
	routeMap["/api/share/delete/batch"] = this.Wrap(this.DeleteBatch, model.USER_ROLE_USER)
	routeMap["/api/share/detail"] = this.Wrap(this.Detail, model.USER_ROLE_USER)
	routeMap["/api/share/page"] = this.Wrap(this.Page, model.USER_ROLE_USER)
	routeMap["/api/share/browse"] = this.Wrap(this.Browse, model.USER_ROLE_GUEST)
	routeMap["/api/share/zip"] = this.Wrap(this.Zip, model.USER_ROLE_GUEST)

	routeMap["/api/share/matter/page"] = this.Wrap(this.MatterPage, model.USER_ROLE_GUEST)
	routeMap["/api/share/matter/preview"] = this.WrapPure(this.MatterPreview, model.USER_ROLE_GUEST)
	routeMap["/api/share/matter/download"] = this.WrapPure(this.MatterDownload, model.USER_ROLE_GUEST)
...
func (this *ShareController) Create(writer http.ResponseWriter, request *http.Request) *result.WebResult {
	...
	if len(uuidArray) > model.SHARE_MAX_NUM {
