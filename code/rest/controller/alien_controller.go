...
func (this *AlienController) RegisterRoutes() map[string]func(writer http.ResponseWriter, request *http.Request) {
	...
	routeMap["/api/alien/fetch/upload/token"] = this.Wrap(this.FetchUploadToken, model.USER_ROLE_USER)
	routeMap["/api/alien/fetch/download/token"] = this.Wrap(this.FetchDownloadToken, model.USER_ROLE_USER)
	routeMap["/api/alien/confirm"] = this.Wrap(this.Confirm, model.USER_ROLE_USER)
	routeMap["/api/alien/upload"] = this.Wrap(this.Upload, model.USER_ROLE_GUEST)
	routeMap["/api/alien/crawl/token"] = this.Wrap(this.CrawlToken, model.USER_ROLE_GUEST)
	routeMap["/api/alien/crawl/direct"] = this.Wrap(this.CrawlDirect, model.USER_ROLE_USER)
