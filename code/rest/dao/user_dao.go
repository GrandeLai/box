...
func (this *UserDao) FindAnAdmin() *model.User {

	var user = &model.User{}
	db := core.CONTEXT.GetDB().Where(&model.User{Role: model.USER_ROLE_ADMINISTRATOR}).First(user)
	if db.Error != nil {
		if db.Error.Error() == result.DB_ERROR_NOT_FOUND {
			return nil
		} else {
...
func (this *UserDao) PageHandle(username string, status string, fun func(user *model.User, space *model.Space)) {
...
func (this *UserDao) Cleanup() {
	...
	db := core.CONTEXT.GetDB().Where("uuid is not null and role != ?", model.USER_ROLE_ADMINISTRATOR).Delete(model.User{})
	this.PanicError(db.Error)
}
