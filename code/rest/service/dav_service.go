...
func (this *DavService) PropstatsFromXmlNames(user *model.User, space *model.Space, matter *model.Matter, xmlNames []xml.Name) []dav.Propstat {
	...
	for _, xmlName := range xmlNames {
		...
		if liveProp := model.LivePropMap[xmlName]; liveProp.FindFn != nil && (liveProp.Dir || !matter.Dir) {
...
func (this *DavService) AllPropXmlNames(matter *model.Matter) []xml.Name {

	pnames := make([]xml.Name, 0)
	for pn, prop := range model.LivePropMap {
...
func (this *DavService) Propstats(user *model.User, space *model.Space, matter *model.Matter, propfind *dav.Propfind) []dav.Propstat {
...
// list the directory.
func (this *DavService) HandlePropfind(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
	...
	if depth == 0 {
		matters = []*model.Matter{matter}
	} else {
		// len(matters) == 0 means empty directory
		matters = this.matterDao.FindByPuuidAndUserUuidAndDeleted(matter.Uuid, user.Uuid, model.FALSE, nil)

		//add this matter to head.
		matters = append([]*model.Matter{matter}, matters...)
	}
	...
	for _, matter := range matters {
		...
		visitPath := fmt.Sprintf("%s%s", model.WEBDAV_PREFIX, matter.Path)
...
func (this *DavService) HandleProppatch(writer http.ResponseWriter, request *http.Request, user *model.User, subPath string) {
...
// handle download
func (this *DavService) HandleGetHeadPost(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
...
// upload a file
func (this *DavService) HandlePut(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
...
// delete file
func (this *DavService) HandleDelete(w http.ResponseWriter, r *http.Request, user *model.User, space *model.Space, subPath string) {
...
// crate a directory
func (this *DavService) HandleMkcol(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
	...
	dbMatter := this.matterDao.FindBySpaceNameAndPuuidAndDirAndName(space.Name, dirMatter.Uuid, model.TRUE, thisDirName)
	if dbMatter != nil {
	...
	fileMatter := this.matterDao.FindBySpaceNameAndPuuidAndDirAndName(space.Name, dirMatter.Uuid, model.FALSE, thisDirName)
	if fileMatter != nil {
...
// cors options
func (this *DavService) HandleOptions(w http.ResponseWriter, r *http.Request, user *model.User, space *model.Space, subPath string) {
...
func (this *DavService) prepareMoveCopy(
	...
	user *model.User, space *model.Space, subPath string) (
	srcMatter *model.Matter,
	destDirMatter *model.Matter,
	srcDirPath string,
	...
	if strings.HasPrefix(destinationStr, model.WEBDAV_PREFIX) {
	...
	if len(strs) == 2 {
		...
		panic(result.BadRequest("destination prefix must be %s", model.WEBDAV_PREFIX))
	}
	...
	if srcMatter.Uuid == model.MATTER_ROOT {
...
// move or rename.
func (this *DavService) HandleMove(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
...
// copy file/directory
func (this *DavService) HandleCopy(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
...
func (h *DavService) stripPrefix(p string) (string, int, error) {
	if r := strings.TrimPrefix(p, model.WEBDAV_PREFIX); len(r) < len(p) {
...
// lock.
func (this *DavService) HandleLock(w http.ResponseWriter, r *http.Request, user *model.User, subPath string) {
...
// unlock
func (this *DavService) HandleUnlock(w http.ResponseWriter, r *http.Request, user *model.User, subPath string) {
...
func (this *DavService) HandleDav(writer http.ResponseWriter, request *http.Request, user *model.User, space *model.Space, subPath string) {
